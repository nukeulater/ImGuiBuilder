...
const char* k_console_window_name = "console";

ConsoleCommand console_opacity_var_cmd("var_console_opacity", "set console opacity, 1 parameter(s): <float>", 1, 1, ImConsoleImpl::set_opacity_cb);

ImConsoleImpl* GetMainConsoleInstance()
{
	static std::unique_ptr<ImConsoleImpl> console(std::make_unique<ImConsoleImpl>());
	return console.get();
}

ImConsoleImpl::ImConsoleImpl() :
	...
	m_console_opacity_comvar(&m_console_opacity)
{
	...
	CommandCollection::InsertCommand(new ConsoleCommand("clear", "clear the output of the current console and history, 0 parameter(s)", 0, 0, ImConsoleImpl::clear_cb));
}

int ImConsoleImpl::LogToMainTabCb(StringHeaderFlags flags, const char* fmt, ...)
{
	va_list valist;
	va_start(valist, fmt);
...
// ImGui text callback
int ImConsoleImpl::TextEditCallback(ImGuiInputTextCallbackData* data)
{
	ImConsoleImpl* console_data = (ImConsoleImpl*)data->UserData;
		...
		else if (output_buffer->GetHeaderCount() > 0)
		{
			...
			if (prev_history_index != console_data->m_history_string_index)
			{
				if (console_data->m_history_string_index >= 0)
				{
					...
					const int count = (int)(output_buffer->GetHeaderCount() - 1);
...
void ImConsoleImpl::ClearMainOutput() 
{
	GetMainConsoleInstance()->GetMainOutput()->Clear();
};

void ImConsoleImpl::ExecCommand(const char* command_line, size_t command_line_length)
{
	m_history_string_index = -1;
...
void ImConsoleImpl::AllocateCompletionCandidatesBuf(unsigned int candidates_count)
{
...
void ImConsoleImpl::DiscardCompletionCandidatesBuf()
{
...
void ImConsoleImpl::SwitchToTab(ConsoleTabs tab)
{
	m_selected_tab_dirty = true;
	m_selected_tab = tab;
}

void ImConsoleImpl::Draw(const char* title, bool* p_open)
{
	if (!*p_open) return;
...
// clear command
int ImConsoleImpl::clear_cb(const std::vector<std::string>& tokens, ConsoleCommandCtxData cbData)
{
	ClearMainOutput();
	return 0;
}
...
// set opacity command
int ImConsoleImpl::set_opacity_cb(const std::vector<std::string>& tokens, ConsoleCommandCtxData cbData)
{
